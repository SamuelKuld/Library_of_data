...
def get_summary_as_string(data):
    return '\n'.join((f"Total Data Points: {len(data)}",
                      ...
                      f"Standard Deviation of temperature : {get_standard_deviation_of_temp(get_type('temperature',data))}",
                      f"Standard Deviation of felt temperature : {get_standard_deviation_of_feeling(get_type('feels-like',data))}",
                      f"Average Temperature : {get_average_temperature(get_type('temperature',data))}",
                      f"Average Felt Temperature : {get_average_feeling_temperature(get_type('feels-like',data))}",
                      f"Minimum Temperature : {min(get_data_without_zero(get_type('temperature',data), 'temperature'))}",
                      f"Max Temperature : {max([value['temperature'] for key, value in get_type('temperature',data).items()])}",
                      f"Minimum Felt Temperature : {min(get_data_without_zero(get_type('feels-like',data), 'feels-like'))}",
